...
def IoU(boxes_pred, boxes_labels, box_format=""):
	...
	if box_format == 'midpoint':
		...
		pred_x2 = boxes_pred[:, 2] + boxes_pred[:, 2] / 2
		pred_y2 = boxes_pred[:, 3] + boxes_pred[:, 3] / 2

		labels_x1 = boxes_labels[:, 0] - boxes_labels[:, 2] / 2
		labels_y1 = boxes_labels[:, 1] - boxes_labels[:, 3] / 2
		labels_x2 = boxes_labels[:, 2] + boxes_labels[:, 2] / 2
		labels_y2 = boxes_labels[:, 3] + boxes_labels[:, 3] / 2
